$(function () {
  ...
  var search_types = [
    ...
    { url: 'https://minecraft.fandom.com/zh/wiki/Special:%E6%90%9C%E7%B4%A2?query=', type: 'mcwiki' },
  ];
...
// 如果没有logo，这里将显示标题的第一个字

document.addEventListener('DOMContentLoaded', function() {
    const logos = document.querySelectorAll('.linklogo');

    logos.forEach(logo => {
        const title = logo.parentElement.querySelector('.title').textContent;
        const firstChar = title.charAt(0);
        logo.textContent = firstChar;
    });
});
...
function updateTime() {
    ...
    document.getElementById('time-display').textContent = `${month}月${day}日 星期${week} ${timeString}`;
}
